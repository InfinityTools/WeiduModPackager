...
| [`multi-tag-x86`](../../tree/multi-tag) | GitHub action template which includes setup binaries and scripts for all supported platforms (Windows, Linux and macOS) into a single mod package. It includes a 32-bit setup binary for Windows and uses the release tag name as version suffix for mod package names. |
